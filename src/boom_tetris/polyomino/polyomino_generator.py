...
Notes:
    ...
    does not generate a 'new' tetromino. - If the fixed size is set to, for
    example, 4, then it will generate all polyominoes with size 4, which are
    called tetrominoes.
"""
...
class PolyominoGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            directions: Named step vectors used to find neighbouring cells. Each
                value is a two- element list ``[dx, dy]``.
        """
    ...
    def _normalize(self, polyomino_shape: tuple[Cell, ...]) -> tuple[Cell, ...]:
        ...
        1. Find the minimum x and minimum y across all cells. 2. Subtract those
        minima from every cell so the leftmost column becomes x=0 and the bottom
        row becomes y=0. 3. Return the cells as a sorted tuple so the
        representation is stable.

        Args:
            polyomino_shape: Occupied cells as a tuple of ``(x, y)`` integer
                pairs.
    ...
    def _rotate(self, polyomino_shape: tuple[Cell, ...]) -> tuple[Cell, ...]:
        ...
        Args:
            polyomino_shape: A normalised, sorted tuple of ``(x, y)`` cell
                pairs.
    ...
    def _obtain_all_rotations(
        ...
        """Return the four normalised orientations produced by successive 90°
        rotations.
        ...
        1. Rotate the input 90° counter-clockwise and normalise; repeat four
        times. 2. Collect each normalised rotation and return all four.

        Args:
            polyomino_shape: Occupied cells as a normalised tuple of ``(x, y)``
                pairs.
    ...
    def generate(self, polyomino_shape: set[Cell] = {(0, 0)}) -> set[tuple[Cell, ...]]:
        """Recursively grow a polyomino and record each unique shape at the
        target size.
        ...
        reaches the target size, it is normalised and checked against all four
        of its rotations; if none of those rotations is already recorded, the
        shape is added to ``unique_shapes``.

        1. Check whether the current shape has reached the target cell count. 2.
        If so, normalise the shape and compute its four rotations. 3. If none of
        the rotations appear in ``unique_shapes``, record the normalised shape.
        4. If the target has not been reached, iterate over every occupied cell
        and every direction; skip neighbours already in the shape; recurse with
        each new candidate set.
