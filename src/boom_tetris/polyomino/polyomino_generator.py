...
Notes:
    - In the case for Tetris, distinct polyominoes means rotations do not count, but
      inversions do. For example, the L-piece and the J-piece are inversions and two
      separate tetromino pieces. However, if you rotate an L-piece, this does not
      generate a 'new' tetromino.
    - If the fixed size is set to, for example, 4, then it will generate all polyominoes
      with size 4, which are called tetrominoes.
"""
...
class PolyominoGenerator:
    """Enumerate one-sided polyominoes of a fixed size using backtracking and rotation-
    invariant deduplication.

    A one-sided polyomino is a connected shape made of unit squares on a grid, where two
    shapes are considered identical if one can be rotated into the other, but mirror
    images are counted as distinct. This generator grows shapes cell-by-cell via DFS and
    discards any new shape whose canonical form matches any rotation of a shape already
    seen.
    """
    ...
    def __init__(
        ...
        Args:
            number_of_polyomino_cells: The exact number of cells each polyomino must
                contain (e.g. 4 for tetrominoes, 5 for pentominoes, etc.).
            directions: Named step vectors used to find neighbouring cells. Each value
                is a two- element list ``[dx, dy]``.
        """
    ...
    def _normalize(self, polyomino_shape: tuple[Cell, ...]) -> tuple[Cell, ...]:
        """Translate a shape so its bounding box starts at the origin, then sort its
        cells.

        Normalisation gives every shape a unique positional representation independent
        of where on the grid it was constructed, which is a prerequisite before any
        rotation-based comparison can be made.

        1. Find the minimum x and minimum y across all cells.
        2. Subtract those minima from every cell so the leftmost column becomes x=0 and
           the bottom row becomes y=0.
    ...
    def _rotate(self, polyomino_shape: tuple[Cell, ...]) -> tuple[Cell, ...]:
        ...
        The transformation ``(x, y) → (y, −x)`` rotates each cell around the origin by
        90° counter- clockwise. Calling this method four times in succession returns the
        shape to its original orientation.
    ...
    def _obtain_all_rotations(
        ...
        Because ``directions`` has four entries, a shape has at most four distinct
        orientations. Shapes with rotational symmetry will produce duplicate entries in
        the list, but that does not affect correctness — the caller only checks
        membership.
    ...
    def generate(self, polyomino_shape: set[Cell] = {(0, 0)}) -> set[tuple[Cell, ...]]:
        ...
        ``generate`` is called again on that candidate. When the candidate reaches the
        target size, it is normalised and checked against all four of its rotations; if
        none of those rotations is already recorded, the shape is added to
        ``unique_shapes``.
        ...
        3. If none of the rotations appear in ``unique_shapes``, record the normalised
           shape.
        4. If the target has not been reached, iterate over every occupied cell and
           every direction; skip neighbours already in the shape; recurse with each new
           candidate set.

        Args:
            polyomino_shape: The set of cells occupied by the polyomino so far. Defaults
